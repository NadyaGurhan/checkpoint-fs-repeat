...
function getPadawanNames(fileName) {
  const text = fs.readFileSync(fileName = './data/padawans.txt', 'utf8').split('\n');
  text.pop();
  return text;
}
...
function getStats() {
  const score = getLightsaberScores('./data/scores.txt', 'utf8');
  const padavans = getPadawanNames('./data/padawans.txt', 'utf8');
